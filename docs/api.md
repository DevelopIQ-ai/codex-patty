...
2. The server publishes exactly the caller's tools and nothing else. Its only authority is a random per-turn session token, passed to it in its environment; with it, it can reach `/internal/tool-bridge/*` on loopback and nothing else in Patty. It is never given a Patty API key, and the token dies with the turn.
3. When the model calls a tool, the invocation becomes an ordinary OpenAI `tool_calls` answer (`finish_reason: "tool_calls"`), and the Codex turn stays open, parked mid-flight.
