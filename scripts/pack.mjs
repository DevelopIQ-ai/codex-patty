#!/usr/bin/env node
// Builds the single publishable `@puffle/pattystack` package from the workspace outputs.
...
// \`npx @puffle/pattystack\` starts the daemon; anything else is a CLI command, so one
...
`;
await writeFile(join(out, 'bin/pattystack.mjs'), launcher);
await chmod(join(out, 'bin/pattystack.mjs'), 0o755);

await writeFile(join(out, 'package.json'), `${JSON.stringify({
  name: '@puffle/pattystack',
  version,
  description: 'Stack your own Codex/ChatGPT subscriptions behind one local, OpenAI-compatible endpoint.',
  license: 'MIT',
  type: 'module',
  engines: { node: '>=22.5' },
  bin: { pattystack: './bin/pattystack.mjs', patty: './cli/index.js', pattyd: './daemon/main.js' },
...
console.log(JSON.stringify({ packed: out, version, bins: ['pattystack', 'patty', 'pattyd'] }));
