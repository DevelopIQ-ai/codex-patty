...
// `--fake` stacks one demo sub; repeat `--fake=<alias>[:<quotaRemaining>[:<minutesUntilReset>]]` to stack several and watch routing choose between them, including the use-it-or-lose-it preference for a window about to roll over.
for (const argument of process.argv.filter(value => value === '--fake' || value.startsWith('--fake='))) {
  const [alias = 'fake-primary', remaining, resetInMinutes] = argument.slice('--fake'.length).replace(/^=/, '').split(':');
  const account = daemon.addFakeAccount(alias || 'fake-primary', ['gpt-5-codex'], remaining === undefined ? 1 : Number(remaining));
