...
else { process.exitCode=1; console.error('usage: patty init <key> | accounts add|list|refresh|remove | models | status | doctor | thread <model> [account] | turn <thread> <model> <input> | events <run> | approve <run> <approval> yes|no | run <model> <input> | cancel <run>'); }
